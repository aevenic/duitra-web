...
const CardFeature = ({ features, activeIndex, language }) => {
	return (
		<div className="md:absolute md:bottom-12 md:right-4 xl:right-0 z-30 w-full md:max-w-[450px] px-4 pb-4 pt-6 rounded-lg bg-neutral-900/80 backdrop-blur-2xl border border-white/5 shadow-2xl transition-all duration-500">
				...
				<div className="flex justify-between items-center w-full">
					...
					<div className="flex gap-x-2">
						{features.map((_, index) => (
							<div
								key={index}
								className={`h-2 transition-all duration-500 rounded-full ${index === activeIndex ? 'w-4 bg-blue-500' : 'w-2 bg-neutral-700'}`}
							/>
						))}
					</div>
				</div>
